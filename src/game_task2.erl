...
task2(GSPid, Context, TaskId, PlayerId, PlayerLoc, {Triggers, _Paths} = GameMap, Status) ->
	...
	task2({patrol, init, [TrigId], []}, GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status).

%%
%% Local Functions
%%
task2({patrol, init, TriggerIds, VisitedTriggerIds}, GSPid, Context, TaskId, PlayerId, PlayerLoc, {Triggers, Paths} = GameMap, Status) ->
	{_SessionId, _SId, _AId, UserId} = Context,
	[TrigId | Ts] = TriggerIds,
	IsVisited = lists:member(TrigId, VisitedTriggerIds),
	if
		IsVisited ->
			task2({patrol, init, Ts, VisitedTriggerIds}, GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status);
		true ->
			{_, {_TrigX, _TrigY} = Trig} = lists:keyfind(TrigId, 1, Triggers),
			%io:format("~p: do_task2 send move x=~p, y=~p~n", [UserId, _TrigX, _TrigY]),
			{X, Y} = offset(UserId, Trig),
			...
			task2({patrol, wait, TriggerIds1, [TrigId | VisitedTriggerIds]},
				GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status)
	end;

task2({patrol, wait, TriggerIds, VisitedTriggerIds}, GSPid, Context, TaskId, PlayerId, PlayerLoc, {Triggers, _Paths} = GameMap, Status) ->
	{_SessionId, _SId, _AId, UserId} = Context,
	receive
		{GSPid, #msg_MoveNotif{
			id = PlayerId, x = X, y = Y}} ->
			...
			Loc2 = offset(UserId, TrigLoc),
			Dist = dist_vec2d(PlayerLoc1, Loc2),
			if
				Dist < ?NEAR_DIST ->
					timer:sleep(?MOVE_CD),
					%io:format("~p: do_task2 arrived x=~p, y=~p~n", [UserId, X, Y]),
					task2({patrol, init, TriggerIds, VisitedTriggerIds},
						GSPid, Context, TaskId, PlayerId, PlayerLoc1, GameMap, Status);
				true ->
					{Loc2X, Loc2Y} = Loc2,
			...
			if
				R ->
					task2({patrol, wait, TriggerIds, VisitedTriggerIds},
						GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status);
				true ->
					%io:format("~p: do_task2: discover a monster, monId=~p, x=~p, y=~p~n", [UserId, MonId, MonX, MonY]),
					task2({attack, init, [{MonId, {MonX, MonY}}], MonId},
						GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status)
			end
	after 2000 ->
		%% timeout then resend move instruction
		%io:format("~p: do_task2 patrol failed moving, retry~n", [UserId]),
		[VTrigId | _Vs] = VisitedTriggerIds,
		{_, TrigLoc} = lists:keyfind(VTrigId, 1, Triggers),
		Loc2 = offset(UserId, TrigLoc),
		{Loc2X, Loc2Y} = Loc2,
...
task2({attack, init, Monsters, TargetMonsterId}, GSPid, Context, TaskId, PlayerId, PlayerLoc, {Triggers, _Paths} = GameMap, Status) ->
	{_SessionId, _SId, _AId, UserId} = Context,
	...
	AttackLoc = get_attack_loc(UserId, PlayerLoc, TrigId, GameMap),
	{LocX, LocY} = AttackLoc,
	%io:format("~p: do_task2 send attack move x=~p, y=~p~n", [UserId, LocX, LocY]),
	GSPid ! {move, #pose{x = LocX, y = LocY, angle = 0.0}},
	task2({attack, {wait, undefined}, Monsters, TargetMonsterId, AttackLoc},
		GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status);

task2({attack, {wait, AttackPid}, Monsters, TargetMonsterId, AttackLoc}, GSPid, Context, TaskId, PlayerId, PlayerLoc, {Triggers, _Paths} = GameMap, Status) ->
	{_SessionId, _SId, _AId, UserId} = Context,
	receive
		{GSPid, #msg_CreatureAppearNotif{
			career = Career,
			...
			%io:format("~p: do_task2: discover a monster when waiting attack, monId=~p, x=~p, y=~p~n", [UserId, MonId, MonX, MonY]),
			R1 = lists:member(Career, [1,2,3]),
			R2 = lists:keyfind(MonId, 1, Monsters),
			case {R1, R2} of
				{false, false} ->
					%io:format("~p: do_task2: this monster(~p) is new~n", [UserId, MonId]),
		...
		{GSPid, #msg_CreatureDisappearNotif{id = MonId}} = _Msg ->
			%io:format("~p: monster killed id=~p~n", [UserId, MonId]),
			case AttackPid of
				undefined -> void;
				_ ->
					if
						MonId == TargetMonsterId ->
							%io:format("~p: do_task2 notify its timer(~p) monster(~p) killed~n", [UserId, AttackPid, MonId]),
					...
					case Monsters1 of
						[] ->
							...
							task2({patrol, init, [NearTrigId], []},
								GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status1);
						_ ->
							if
								MonId == TargetMonsterId ->
	...
	after 2000 ->
		...
		%io:format("~p: do_task2 failed moving, retry~n", [UserId]),
		{AttackLocX, AttackLocY} = AttackLoc,
		GSPid ! {move, #pose{x = AttackLocX, y = AttackLocY, angle = 0.0}},
		task2({attack, {wait, AttackPid}, Monsters, TargetMonsterId, AttackLoc},
			GSPid, Context, TaskId, PlayerId, PlayerLoc, GameMap, Status)
...
attack_monster(TaskPid, UserId, GSPid, MonsterId) ->
	receive
		{TaskPid, {killed, _MonId}} ->
			%io:format("~p: attack_monster received monster(~p) killed! killing(~p)~n", [UserId, _MonId, MonsterId]),
			void;	% quit the loop
		_Msg ->
...
get_attack_loc(UserId, PlayerLoc, TrigId, {Triggers, Paths}) ->
